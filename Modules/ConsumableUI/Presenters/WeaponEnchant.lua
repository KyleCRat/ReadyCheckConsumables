...
function WeaponEnchant.Present(model)
    ...
    if selected.kind == "spell" then
        ...
        if candidate then
            if not active then state.icon = selected.icon end
            ...
            if selected.unavailable then
                if model.hasEnchant then State.SetHoverUnavailable(state, OUT_OF_SELECTED_ITEM)
                else State.SetUnavailable(state, OUT_OF_SELECTED_ITEM) end
            end
...
function WeaponEnchant.Choices(selected)
    ...
    if selected.kind ~= "spell" then
        ...
        for _, spell in ipairs(selected.spells) do
            if spell.data ~= selected.active and spell.action then
                choices[#choices + 1] = {
                    icon = spell.icon, desaturated = false, countText = "",
                    tooltipSpellID = spell.data.spellID,
                    clickHintSpellID = spell.data.spellID, action = spell.action,
                }
            end
        end
    end
    ...
    local itemChoices = State.CreateItemFlyoutChoices(selected.candidates,
        candidate and candidate.itemID, {
            targetSlot = selected.slotID, available = true,
            preferenceKey = selected.preferenceKey,
            includeSingleChoice = selected.kind == "spell" or selected.unavailable,
        })
    for _, choice in ipairs(itemChoices or {}) do choices[#choices + 1] = choice end
