...
function Effects.Aura(aura, context, now)
    ...
    local bad = remaining ~= nil and remaining <= context.warningSeconds
    return {
...
function Effects.AddDeadline(model, expiry, context, now)
    ...
    local warning = expiry - context.warningSeconds
...
function Effects.Evaluate(selection, observation, context, now)
    local effect = Effects.Aura(observation.aura, context, now)
    local model = { selection = selection, action = selection.action, effect = effect, available = observation.available }
    if effect then Effects.AddDeadline(model, effect.expiry, context, now) end
    return model
end
